...
class MediaLogClient : public ServiceClient {
 public:
    ...
    std::shared_future<StoreResultType> StoreAsync(
        const std::string& camera_name, const RPCParameters& parameters = RPCParameters());

    StoreResultType Store(const std::string& camera_name, const std::string& tag,
                          const RPCParameters& parameters = RPCParameters());

    StoreResultType Store(const std::string& camera_name,
                          const RPCParameters& parameters = RPCParameters());
    ...
    StoreResultType Store(::bosdyn::api::spot_cam::StoreRequest& request,
                          const RPCParameters& parameters = RPCParameters());

    std::shared_future<GetStatusResultType> GetStatusAsync(
        const std::string& logpoint_name, const RPCParameters& parameters = RPCParameters());

    GetStatusResultType GetStatus(const std::string& logpoint_name,
                                  const RPCParameters& parameters = RPCParameters());

    std::shared_future<GetStatusResultType> GetStatusAsync(
        ::bosdyn::api::spot_cam::GetStatusRequest& request,
        const RPCParameters& parameters = RPCParameters());
    GetStatusResultType GetStatus(::bosdyn::api::spot_cam::GetStatusRequest& request,
                                  const RPCParameters& parameters = RPCParameters());

    std::shared_future<TagResultType> TagAsync(const std::string& logpoint_name,
                                               const std::string& tag,
                                               const RPCParameters& parameters = RPCParameters());

    TagResultType Tag(const std::string& logpoint_name, const std::string& tag,
                      const RPCParameters& parameters = RPCParameters());

    std::shared_future<TagResultType> TagAsync(::bosdyn::api::spot_cam::TagRequest& request,
                                               const RPCParameters& parameters = RPCParameters());

    TagResultType Tag(::bosdyn::api::spot_cam::TagRequest& request,
                      const RPCParameters& parameters = RPCParameters());
    ...
    DebugResultType EnableDebug(::bosdyn::api::spot_cam::DebugRequest& request,
                                const RPCParameters& parameters = RPCParameters());

    std::shared_future<ListCamerasResultType> ListCamerasAsync(
        const RPCParameters& parameters = RPCParameters());

    ListCamerasResultType ListCameras(const RPCParameters& parameters = RPCParameters());

    std::shared_future<DeleteResultType> DeleteAsync(
        const std::string& logpoint_name, const RPCParameters& parameters = RPCParameters());

    DeleteResultType Delete(const std::string& logpoint_name,
                            const RPCParameters& parameters = RPCParameters());
    ...
    DeleteResultType Delete(::bosdyn::api::spot_cam::DeleteRequest& request,
                            const RPCParameters& parameters = RPCParameters());
    ...
    // Get the default service name the spot cam media log service will be registered in the
    // directory with.
    static std::string GetDefaultServiceName() { return s_default_service_name; }

    // Get the default service type for the spot cam media log service that will be registered in
    // the directory.
...
RetrieveResultType Retrieve(::bosdyn::api::spot_cam::RetrieveRequest& request, const RPCParameters& parameters = RPCParameters());
 ...
 private:
    ...
    void OnStoreComplete(MessagePumpCallBase* call,
                         ...
                         const grpc::Status& status, std::promise<StoreResultType> promise);
    ...
    void OnTagComplete(MessagePumpCallBase* call,
    ...
    void OnEnableDebugComplete(MessagePumpCallBase* call,
                               const ::bosdyn::api::spot_cam::DebugRequest& request,
                               ::bosdyn::api::spot_cam::DebugResponse&& response,
                               const grpc::Status& status, std::promise<DebugResultType> promise);
    void OnListCamerasComplete(MessagePumpCallBase* call,
                               const ::bosdyn::api::spot_cam::ListCamerasRequest& request,
                               ::bosdyn::api::spot_cam::ListCamerasResponse&& response,
                               const grpc::Status& status,
                               std::promise<ListCamerasResultType> promise);
    void OnDeleteComplete(MessagePumpCallBase* call,
